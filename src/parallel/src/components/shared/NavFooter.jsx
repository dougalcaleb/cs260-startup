		...
		<div className="fixed rounded-tr-4xl rounded-tl-4xl h-20 w-full bottom-0 left-0 flex justify-center items-center bg-gray-3 sm:hidden z-10">
			{!isComparing && (
				<nav className="bg-gray-1 w-[80%] h-6 rounded-4xl flex justify-between items-center">
					<Link to={!atLibrary ? `${URL_BASE}/library` : null} className={`w-[30%] h-full flex flex-col justify-center rounded-4xl ${atLibrary ? selectedClass : 'text-white-0'}`}>
						<div className="w-full font-main font-black text-center flex justify-center text-xs">YOU</div>
					</Link>
					<Link to={!atNearby ? `${URL_BASE}/nearby` : null} className={`w-[30%] h-full flex flex-col justify-center rounded-4xl ${atNearby ? selectedClass : 'text-white-0'}`}>
						<div className="w-full font-main font-black text-center flex justify-center text-xs">NEARBY</div>
					</Link>
					<Link to={!atSearch ? `${URL_BASE}/search` : null} className={`w-[30%] h-full flex flex-col justify-center rounded-4xl ${atSearch ? selectedClass : 'text-white-0'}`}>
