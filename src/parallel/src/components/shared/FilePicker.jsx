						...
						<div className="flex flex-col mb-4">
							{files.map((f, i) => (
								<div key={`file-display-${inputId}-${i}`} className="font-mono text-gray-8 hover:bg-gray-3 p-1 flex items-center">
									<p >
										<span className="break-all">{f.name}</span> ({formatFileSize(f.size)})
									</p>
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" className="min-h-5 min-w-5 w-5 h-5 sm:w-5 sm:h-5 ml-2 cursor-pointer text-gray-7 hover:text-red-2" onClick={() => removeFile(i)}>
